...
def parseDDRCourse(data):
    return {
        "name": data[0],
        "level": int(data[1]),
        "songs": [{"name": name} for name in data[2:]],
        "source": "DDR",
    }
...
def parseLIFE4Course(data):
    return {
        ...
        "level": int(data[1]),
        "songs": [
            {"name": name, "difficulty": DIFFICULTY[diff]}
            for diff, name in zip(data[2], data[3:])
        ],
        "source": "LIFE4",
    }
...
def main():
    ...
    with open(str(globals.dist_folder / "data" / "courses.json"), "w") as file:
        json.dump(d, file)
    return d
